...
2. **Configure the PHP version**
    ...
    The PHP version is defined in the `cli` service. The default image for the `cli` service is `docksal/cli:2.6-php7.2`. We can change this in `docksal.env` by adding the following variable: `CLI_IMAGE=cli:2.9-php7.3`. This is the latest version of this image tagged with PHP 7.3.
...
{{% notice info %}}
A client wants a decoupled solution to be hosted on two servers. The Drupal side will be hosted on Acquia and the front-end side will be hosted on Netlify. The project is going to use GatsbyJS for the front-end.
{{% /notice %}}
